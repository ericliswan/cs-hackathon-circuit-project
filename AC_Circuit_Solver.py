...
class Wire(Component):
    """
    Concrete subclass of Component representing a jumper wire, which can be
    connected between any two points on the breadboard.
    """
    def __init__(self, name: str, nodeA: int, nodeB: int):
...
class PassiveComponent(Component):
    ...
    def __repr__(self) -> str:
        return f"{self.get_name()}, {self.get_nodes()[0]}->{self.get_nodes()[1]}, {self.get_value()}"
...
def solve_nodal(components: list[Component], omega: float = 0.0) -> tuple[list[complex], dict[str, complex]]:
    ...
    # To handle floating components
    GMIN = 1e-12    
    for i in range(len(nodal_list)):
        coefficients[i][i] += GMIN
...
if __name__ == "__main__":
    ...
    # finds the parameters for the given voltage drop, the first argument is
    # specified by the user (2,0) is given as a tuple input which is left node
    # and right node
