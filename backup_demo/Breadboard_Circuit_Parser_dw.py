from backup_demo.AC_Circuit_Solver_dw import *
import uuid
...
def compact_components(components: list[list[str]]) -> list[list[str]]:
    valid_rows = [row for row in components if len(row) >= 3]
    ys = [[row[0], node(row[1]), node(row[2])] for row in valid_rows]
