...
from backup_demo.Breadboard_Circuit_Parser_dw import *
...
# calculate omega based on the frequency
...
""" did not have time to finish this class :("""
"""
...
class CIRCUIT_OT_animate_wave(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            if waveform_obj and waveform_obj.type == 'CURVE':
                ...
                _, params = self.check_conecctions()
                self.draw_sine_wave(waveform_obj, self._time_offset, params)
                ...
                for area in context.screen.areas:
                    ...
                    print("curve doesn't get drawn?")
        
        return {'PASS_THROUGH'}

    def check_conecctions(self):
        """
        Returns whether the circuit is all connected, and the params of the corresponding cos wave
        """
        asset_terminals = get_asset_terminals()
        board_terminals = get_board_terminals()
        data_to_give = {}
        ...
        # add the probe
        list_to_give.append(self.get_probe_coordinates())
    ...
    def execute(self, context):
        ...
        """
        waveform_obj = bpy.data.objects.get("Waveform")
        checker, params = self.check_conecctions()
        ...
        if checker:
            ...
            self.report({'WARNING'}, "Circuit Incomplete. Check connections and probe.")
            if waveform_obj and waveform_obj.type == 'CURVE':
    ...
    def draw_sine_wave(self, curve_obj, offset, params):
        """
        Updates points with a moving phase offset.
        """
        amplitude, omega, phi = params[0], params[1], params[2]
        ...
        if spline.type == 'BEZIER':
            ...
            for i in range(points_count):
                ...
                print(f"y = {amplitude}sin({omega * i + phi  - (np.pi / 2) + offset})")
                y = amplitude * math.sin((i * 0.5) + phi + offset) * 0.3
    ...
    def get_probe_coordinates(self):
        """
        interactive probe! try test this
        """
        try:
            return ["PROBE", (8,"d"), (68,"b")]
        except:
            self.report({'WARNING'}, "Probe not connected.")
            return {'CANCELLED'}
